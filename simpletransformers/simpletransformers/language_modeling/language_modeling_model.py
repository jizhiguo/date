...
from simpletransformers.language_modeling.language_modeling_utils import (
    DocumentDataset, SimpleDataset, ListOfStrDataset,calculate_acc, get_metrics, mask_tokens,
        ...
        **kwargs,
        ...
        if self.args.model_type == "electra":
            if generator_name:
                ...
                # if new_tokenizer:#whether a new tokenizer or not,always update vocab_size,check disc also.
            ...
            if discriminator_name:
                ...
                # if new_tokenizer:#whether a new tokenizer or not,always update vocab_size,check gen also.
            ...
            if self.args.model_type == "electra":
                if self.args.model_name == "electra":
                    ...
                    self.model = model_class.from_pretrained(
                        model_name,
                        len(self.masks),#add this para for avoiding error loading pretrained model
                        self.extra_args,#add this para for avoiding error loading pretrained model
                        config=self.config,
...
#                        torch.load(
 #                           os.path.join(self.args.model_name,
   #                                      "pytorch_model.bin")))
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc=f"iteration{global_step}", disable=args.silent)):
                ...
                if "vanilla_electra" in self.extra_args and self.extra_args["vanilla_electra"] != False:
                    ...
                    inputs, labels, labels_clf = mask_tokens(
                        ...
                        train=True, no_mask=True) if args.mlm else (batch, batch)
    ...
    def evaluate_anomaly_docs(self,
        ...
        if self.extra_args['eval_anomalies']:
            ...
            anomaly_results_cutoff = {
                "PL_RTD": {
                    "cutoff":cutoff_pl_electra_bug,
                    "pred_scores":np.concatenate((inlier_scores_pl_electra_2, anomaly_scores_pl_electra_2))
                },
            }
    ...
    def predicts(self, list_str:List[str], anom_word_threshold=0.15, anom_word_alpha=100, oov_alpha=100, positive_cutoff=0.8):
        """
            Predicts ``list_str`` on the trained model.

            ARGs:
                ``list_str``字符串列表，每行作为一个句子预测，如果句子长度超过最长限制，自动分成多个句子。
                ``anom_word_threshold``怪词置信度阈值（0~1实数）：例如0.15，置信度小于设定值判为怪词。
                ``anom_word_alpha``怪词惩罚系数（0~100实数）：0表示零容忍怪词，数字越大，越容忍怪词。
                ``oov_alpha``OOV惩罚系数（0~100实数）：	0表示零容忍OOV，数字越大，越容忍OOV，OOV通常置信度很高，不宜当作怪词来处理。
                ``positive_cutoff``正例置信度阈值（0~1实数）：例如0.8，	大于该设定值判为正例，反之，判为反例。
            Return:
                results，一个字典类型的返回值。
                字典键值如下。
                ``results["input tokens"]``：               List，每个元素也是一个List，表示的一个句子在tokenization之后的分词列表。
                ``results["input tokens scores"]``：            List，每个元素是一维数组，数组每个元素表示对应Token是正常词的概率
                ``results["input tokens pl score"]``：          一维数组，每个元素表示一个句子是正常句子的概率
                ``results["input seqs representation"]``：      二维数组，行对应句子，列是句子的向量表示
                ``results["input seqs pl score"]``：            一维数组，每个元素表示一个句子被RMD的概率
                ``results["input tokens scores (reversed)"]``： 与``results["input tokens scores"]``结构相同，将special words对应的概率反转至50%以下，
                                                                例如[UNK]概率p>50%，则反转为1-p，因为OOV概率通常很高，而OOV无意义。
                ``results["input tokens pl score (weighted)"]``：与``results["input tokens pl score"]``结构相同，根据容忍怪词和容忍OOV系数加权平均计算评分
                ``results["seq pos or neg (weighted)"]``：      根据阈值``positive_cutoff``参数，``results["input tokens scores (reversed)"]`` >= positive_cutoff，设置为``正常``（Positive），反之，``异常``（Negative）
        """
        assert anom_word_threshold >=0 and anom_word_threshold <=1 ,anom_word_threshold
        assert anom_word_alpha >=0 and anom_word_alpha <=100 ,anom_word_alpha
        assert oov_alpha >=0 and oov_alpha <=100 ,oov_alpha
        assert positive_cutoff >=0 and positive_cutoff <=1 ,positive_cutoff
        ...
        #truncate the str lenth more than max_seq_length - 3
        for idx, str in enumerate(list_str):
            if len(str)>self.args.max_seq_length - 3:
                logger.info(f'truncate the long str to  {self.args.max_seq_length - 3} :{str[0:self.args.max_seq_length - 3]}...')
                list_str[idx] = str[0:self.args.max_seq_length - 3]
            

        pred_dataset = ListOfStrDataset(
            self.tokenizer,
            self.args,
            list_str,
            self.args.block_size,
        ...
        anomaly_scores_pl, anomaly_scores_mp, anomaly_scores_ne, anomaly_scores_pl_electra_2, anomaly_scores_mp_electra_2, anomaly_scores_ne_electra_2 = self.calculate_scores(pred_dataloader, 'predict_sc_list_str')

        input_tokens = [tokenizer.convert_ids_to_tokens(ids.squeeze()) for ids_set in self.current_data for ids in ids_set]
        results["input tokens"] =[list(compress(tokens,~np.isin(tokens,
                                                                [tokenizer.cls_token,
                                                                 tokenizer.sep_token,
                                                                 tokenizer.pad_token]))) for tokens in input_tokens]#remove special tokens
        results["input tokens scores"] =[score[~np.isinf(score)] for score_set in self.current_scores_pl for score in score_set]
        results["input tokens pl score"] = anomaly_scores_pl_electra_2
        results["input seqs representation"] = [repr.squeeze()  for repr_set in self.repr_list for repr in repr_set]
        results["input seqs pl score"] = anomaly_scores_pl
        results["input tokens scores (reversed)"] = [tokens_score.copy() for tokens_score in results["input tokens scores"]]

        #create weight_anorm_word arrays like arrays of tokens_score, and init them with 1s(normal words) , anom_word_threshold(anormlous words),anom_word_alpha(importance)
        #??remove the last elem in each array (exclude the SEP special token's score)
        weight_anorm_word = [np.where(tokens_score < anom_word_threshold, 1 + 2 * (100-anom_word_alpha), 1)[:-1] for tokens_score in results["input tokens scores"]]

        #create weight_oov arrays like arrays of input tokens wite 1s(normal words)
        weight_oov = [np.ones(len(tokens)) for tokens in results["input tokens"]]
        #update weight_oov with oov_alpha(importance) and reverse the special tokens score to below 50% when necessary
        for idx_toks, tokens in enumerate(results["input tokens"]):
            for idx_tok,token in enumerate(tokens):
                 if token in tokenizer.all_special_tokens:
                    weight_oov[idx_toks][idx_tok] = 1 + 2 * (100-oov_alpha)
                    if results["input tokens scores (reversed)"][idx_toks][idx_tok] > 0.5:#reverse the special tokens score to below 50% when necessary
                        results["input tokens scores (reversed)"][idx_toks][idx_tok] = 1 - results["input tokens scores (reversed)"][idx_toks][idx_tok]

        weight = (np.array(weight_anorm_word, dtype=object) + np.array(weight_oov, dtype=object)) / 2
        weighted_score = [np.average(tokens_score[:-1], weights = weight[idx]) for idx, tokens_score in enumerate(results["input tokens scores (reversed)"])]
        results["input tokens pl score (weighted)"] = weighted_score
        results["seq pos or neg (weighted)"] =["正常" if score >= positive_cutoff else "异常" for score in weighted_score]
        
        return results

    def tb_add_embedding(self, eval_file, eval_file_outlier, tag='default'):
        if not (os.path.exists(eval_file) and os.path.exists(eval_file)):
            logger.info(f'missing file: {eval_file} {eval_file_outlier}')
            return
        self.build_text_samples(eval_file, eval_file_outlier) 
        tb_writer = SummaryWriter(logdir=self.args.tensorboard_dir)
        
        results = self.predicts(self.lines)
        
        from datetime  import datetime
        now = datetime.now()
        date_time = now.strftime("%m%d%Y_%H%M%S")
        metadata=["normal" if idx < self.inlier_len else "anormaly" for idx,value in enumerate(results["input tokens"])]

        tb_writer.add_embedding(
            # torch.tensor(results["input seqs representation"]),
            torch.tensor(np.array(results["input seqs representation"])),
            metadata=metadata,
            label_img=None,
            global_step=None, 
            tag=tag+date_time, 
