...
class ListOfStrDataset(Dataset):
    def __init__(self, tokenizer, args, list_str, block_size=514, special_tokens_count=2, sliding_window=False):
        logger.info(f" ListOfStrDataset - data:{len(list_str)}")
        self.tokenizer = tokenizer
        block_size = block_size - special_tokens_count
